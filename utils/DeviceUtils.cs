...
namespace AimRobotLite.utils {
    public class DeviceUtils {
        ...
        static DeviceUtils() {
            SCREEN_SIZE = GetPrimaryScreenResolution();
        }

        [DllImport("user32.dll", SetLastError = true)]
        public static extern void SendInput(uint cInputs, [MarshalAs(UnmanagedType.LPArray), In] INPUT[] pInputs, int cbSize);
        ...
        public static void KeyPress(ScanCodeShort a) {
            KeyDown(a, false);
            KeyUP(a, false);
        }

        public static void MouseMove(int x, int y) {
            int screenX = (int)(((double)x / SCREEN_SIZE[0]) * 65535);
            int screenY = (int)(((double)y / SCREEN_SIZE[1]) * 65535);

            INPUT[] inputs = new INPUT[1];
            INPUT input = new INPUT();
            input.type = INPUTTYPE.INPUT_MOUSE;
            input.U.mi.dwFlags = MOUSEEVENTF.MOVE | MOUSEEVENTF.ABSOLUTE;
            input.U.mi.dx = screenX;
            input.U.mi.dy = screenY;

            inputs[0] = input;
            SendInput(1, inputs, INPUT.Size);
        }

        public static int[] GetPrimaryScreenResolution() {
